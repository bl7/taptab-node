...
router.post('/', authenticateToken, requireRole(['TENANT_ADMIN', 'MANAGER']), async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const { number, capacity, location, status = 'available' } = req.body;

    if (!number) {
      return sendError(res, 'VALIDATION_ERROR', 'Table number is required', 400);
    }
    ...
    const table = await createWithCheck('tables', tableData, 'number', number, tenantId);
    ...
    sendSuccess(res, { table: formattedTable }, 'Table created successfully', 201);
  } catch (error) {
...
router.put('/:id', authenticateToken, requireRole(['TENANT_ADMIN', 'MANAGER']), async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const { id } = req.params;
    const { number, capacity, status, location, currentOrderId } = req.body;
...
router.delete('/:id', authenticateToken, requireRole(['TENANT_ADMIN', 'MANAGER']), async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const { id } = req.params;
