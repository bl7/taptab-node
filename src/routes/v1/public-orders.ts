...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!tableNumber || !items || !Array.isArray(items) || items.length === 0) {
      return sendError(res, 'VALIDATION_ERROR', 'Table number and items array are required', 400);
    }
    ...
    for (const item of items) {
      ...
      const itemPlaceholders = itemValues.map((_, index) => `$${index + 1}`).join(', ');
    ...
    logger.info(`Public order created: ${order.orderNumber} for table ${tableNumber}`);
